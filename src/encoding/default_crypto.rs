...
impl JoinRequestCreator<[u8; 23], DefaultFactory> {
    ...
    ///
    /// ```
    /// let mut phy = lorawan::encoding::creator::JoinRequestCreator::new();
    /// let key = lorawan::encoding::keys::AES128([7; 16]);
    /// phy.set_app_eui(&[1; 8]);
    /// phy.set_dev_eui(&[2; 8]);
...
impl<T: AsRef<[u8]>> JoinRequestPayload<T, DefaultFactory> {
    ...
    /// let phy = lorawan::encoding::parser::JoinRequestPayload::new(data);
    /// ```
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> DecryptedJoinAcceptPayload<T, DefaultFactory> {
    ...
    /// let key = lorawan::encoding::keys::AES128([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66,
    ///     0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]);
    /// let phy = lorawan::encoding::parser::DecryptedJoinAcceptPayload::new(&mut data[..], &key);
    /// ```
...
impl<T: AsRef<[u8]>> EncryptedDataPayload<T, DefaultFactory> {
    ...
    /// let phy = lorawan::encoding::parser::EncryptedDataPayload::new(data);
    /// ```
    pub fn new(data: T) -> Result<Self, Error> {
        Self::new_with_factory(data, DefaultFactory)
    }
...
impl<T: AsRef<[u8]> + AsMut<[u8]>> DecryptedDataPayload<T> {
    ...
    /// let dec_phy = lorawan::encoding::parser::DecryptedDataPayload::new(data,
    ///     &nwk_skey,
    ///     Some(&app_skey),
    ///     1).unwrap();
    /// ```
