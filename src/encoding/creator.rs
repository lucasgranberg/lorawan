...
impl<D: AsMut<[u8]>, F: CryptoFactory + Default> JoinAcceptCreator<D, F> {
    ...
    /// # Argument
    ///
    /// * app_nonce - instance of lorawan::encoding::parser::AppNonce or anything that can
    ...
    /// # Argument
    ///
    /// * net_id - instance of lorawan::encoding::parser::NwkAddr or anything that can
    ///   be converted into it.
    ...
    /// # Argument
    ///
    /// * dl_settings - instance of lorawan::encoding::maccommands::DLSettings or anything
    ...
    pub fn set_c_f_list<C: AsRef<[Frequency]>>(&mut self, list: C) -> Result<&mut Self, Error> {
        let ch_list = list.as_ref();
        if ch_list.len() > 5 {
            return Err(Error::CfListTooLong);
        }
...
impl JoinAcceptCreator<[u8; 33], DefaultFactory> {
    ...
    ///
    /// ```
    /// let mut phy = lorawan::encoding::creator::JoinAcceptCreator::new();
    ...
    /// phy.set_rx_delay(1);
    /// let mut freqs: Vec<lorawan::encoding::maccommands::Frequency> = Vec::new();
    /// freqs.push(lorawan::encoding::maccommands::Frequency::new(&[0x58, 0x6e, 0x84,]).unwrap()).unwrap();
    /// freqs.push(lorawan::encoding::maccommands::Frequency::new(&[0x88, 0x66, 0x84,]).unwrap()).unwrap();
...
///
/// ```
/// let mut phy = lorawan::encoding::creator::DataPayloadCreator::new();
/// let nwk_skey = lorawan::encoding::keys::AES128([2; 16]);
/// let app_skey = lorawan::encoding::keys::AES128([1; 16]);
/// phy.set_confirmed(true)
///     .set_uplink(true)
///     .set_f_port(42)
///     .set_dev_addr(&[4, 3, 2, 1])
///     .set_fctrl(&lorawan::encoding::parser::FCtrl::new(0x80, true)) // ADR: true, all others: false
///     .set_fcnt(76543);
...
impl<D: AsMut<[u8]>, F: CryptoFactory + Default> DataPayloadCreator<D, F> {
    ...
    /// ```
    /// let mut phy = lorawan::encoding::creator::DataPayloadCreator::new();
    /// let mac_cmd1 = lorawan::encoding::maccommands::UplinkMacCommand::LinkCheckReq(
    ///     lorawan::encoding::maccommands::LinkCheckReqPayload());
    /// let mut mac_cmd2 = lorawan::encoding::maccommandcreator::LinkADRAnsCreator::new();
    /// mac_cmd2
    ///     .set_channel_mask_ack(true)
    ///     .set_data_rate_ack(false)
    ///     .set_tx_power_ack(true);
    /// let mut cmds: Vec<&dyn lorawan::encoding::maccommands::SerializableMacCommand> = Vec::new();
    /// cmds.push(&mac_cmd1);
    /// cmds.push(&mac_cmd2);
    /// let nwk_skey = lorawan::encoding::keys::AES128([2; 16]);
    /// let app_skey = lorawan::encoding::keys::AES128([1; 16]);
...
impl DataPayloadCreator<GenericArray<u8, U256>, DefaultFactory> {
    ...
    /// let nwk_skey = lorawan::encoding::keys::AES128([2; 16]);
    /// let app_skey = lorawan::encoding::keys::AES128([1; 16]);
    /// let fctrl = lorawan::encoding::parser::FCtrl::new(0x80, true);
