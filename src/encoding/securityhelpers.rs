...
pub fn calculate_data_mic<M: keys::Cmac>(data: &[u8], key: M, fcnt: u32) -> keys::MIC {
    let mut header = [0; 16];
...
fn calculate_mic_with_header<M: keys::Cmac>(header: &[u8], data: &[u8], mic: M) -> keys::MIC {
    let mut cipher = mic;
    cipher.input(header);
    cipher.input(data);
...
pub fn calculate_mic<M: keys::Cmac>(data: &[u8], key: M) -> keys::MIC {
